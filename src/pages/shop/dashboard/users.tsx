...
function Index() {
  ...
  const fetchUsers = async () => {
    try {
      ...
      if (!response.data.users || response.data.users.length === 0) return;
...
const UserCard = ({ user }: { user: IUser }) => {
  ...
  return (
    <div className="relative bg-white shadow-md p-6 rounded-lg max-w-sm">
      {/* Three-dot menu */}
      ...
      {/* Role */}
      <p className="mb-4 text-gray-600 text-sm capitalize">{user.role}</p>
